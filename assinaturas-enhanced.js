// assinaturas-enhanced.js v2 — Resumo mensal + Editar assinatura (fix encoding)
(function(){
'use strict';
...
if(!document.getElementById('modalEditSub')){
  ...
  m.innerHTML = '<div class="modal-content"><div class="modal-header">' +
    ...
    '<div class="modal-body">' +
      ...
      '<div class="form-group"><label>Valor/m&ecirc;s (R$)</label><input id="esValor" class="form-control"></div>' +
      '<div class="form-group"><label>Cart&atilde;o</label><select id="esCartao" class="form-control"></select></div>' +
      '<div class="form-group"><label>Categoria</label><select id="esCat" class="form-control"></select></div>' +
      '<div class="form-group"><label>In&iacute;cio</label><input type="month" id="esInicio" class="form-control"></div>' +
...
window._esSave = function(){
  ...
  if(Math.abs(valor - valorAntigo) > 0.01){
    ...
    var d = new Date();
    var mesAtualStr = d.getFullYear() + '-' + String(d.getMonth()+1).padStart(2,'0');
...
window.renderAssinaturas = function(){
  ...
  // ═══ 2. BOTÃO EDITAR EM CADA ASSINATURA ═══
  var subGrid = document.getElementById('subGrid');
  if(!subGrid) return;
